...
use ExpertsCrm\Transactions\CompanyTransaction;
use ExpertsCrm\Validators\Company\CompanyValidator;
...
class CompanyAction {
    ...
    static function list(\WP_REST_Request $request) {
        ...
        $result = CompanyTransaction::list(
            new ListConfig(
                $pointer,
                $query,
                $sort
    ...
    static function view(\WP_REST_Request $request) {
        ...
        try {
            ...
            $doc = CompanyTransaction::view(new ViewConfig($id, $expanded));
    ...
    static function store(\WP_REST_Request $request) {
        ...
        try {
            ...
            $cleanDoc["search_text"] = json_encode($cleanDoc);
            $id = CompanyTransaction::store($cleanDoc, $urlParams["id"] ?? null);
            return new \WP_REST_Response(["company" => $id], 200);
        } catch ( \Exception $e ) {
