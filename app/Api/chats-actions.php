...
add_action( 'rest_api_init', function () {
    register_rest_route( WPB_CHATBOT_API_PREFIX, '/chatbots/(?P<chatbot_id>\d+)/chats', array(
        ...
        'permission_callback' => function ($request) {
            ...
            $result = $auth->list((int) $request->get_param('user_id') ?: get_current_user_id(), $request->get_param('chatbot_id'));
...
add_action( 'rest_api_init', function () {
    register_rest_route( WPB_CHATBOT_API_PREFIX, '/chatbots/(?P<chatbot_id>\d+)/chats/(?P<id>\d+)', array(
        ...
        'permission_callback' => function ($request) {
            $auth = wpb_chatbot_app(\WPBulgaria\Chatbot\Auth\Factory\ChatsAuthFactory::class);
            $result = $auth->get(get_current_user_id(), $request->get_param('id'), $request->get_param('chatbot_id'));
            if ($auth->hasError() && !$result) {
                return new \WP_Error("unauthorized", $auth->getError()->getMessage(), array("status" => 401));
            }
            return $result;
...
add_action( 'rest_api_init', function () {
    register_rest_route( WPB_CHATBOT_API_PREFIX, '/chatbots/(?P<chatbot_id>\d+)/chats/(?P<id>\d+)', array(
        ...
        'permission_callback' => function ($request) {
            $auth = wpb_chatbot_app(\WPBulgaria\Chatbot\Auth\Factory\ChatsAuthFactory::class);
            $result = $auth->updateTitle(get_current_user_id(), $request->get_param('id'), $request->get_param('chatbot_id'));
            if ($auth->hasError() && !$result) {
                return new \WP_Error("unauthorized", $auth->getError()->getMessage(), array("status" => 401));
            }
            return $result;
...
add_action( 'rest_api_init', function () {
    register_rest_route( WPB_CHATBOT_API_PREFIX, '/chatbots/(?P<chatbot_id>\d+)/chats/(?P<id>\d+)', array(
        ...
        'permission_callback' => function ($request) {
            $auth = wpb_chatbot_app(\WPBulgaria\Chatbot\Auth\Factory\ChatsAuthFactory::class);
            $result = $auth->trash(get_current_user_id(), $request->get_param('id'), $request->get_param('chatbot_id'));
            if ($auth->hasError() && !$result) {
                return new \WP_Error("unauthorized", $auth->getError()->getMessage(), array("status" => 401));
            }
            return $result;
...
add_action( 'rest_api_init', function () {
    register_rest_route( WPB_CHATBOT_API_PREFIX, '/chatbots/(?P<chatbot_id>\d+)/chats/(?P<id>\d+)/force', array(
        'methods' => 'DELETE',
        'callback' => 'WPBulgaria\Chatbot\Actions\ChatAction::remove',
        'permission_callback' => function ($request) {
            $auth = wpb_chatbot_app(\WPBulgaria\Chatbot\Auth\Factory\ChatsAuthFactory::class);
            $result = $auth->remove(get_current_user_id(), $request->get_param('id'), $request->get_param('chatbot_id'));
            if ($auth->hasError() && !$result) {
                return new \WP_Error("unauthorized", $auth->getError()->getMessage(), array("status" => 401));
            }
            return $result;
...
add_action( 'rest_api_init', function () {
    register_rest_route( WPB_CHATBOT_API_PREFIX, '/chatbots/(?P<chatbot_id>\d+)/chats/(?P<id>\d+)/restore', array(
        'methods' => 'POST',
        'callback' => 'WPBulgaria\Chatbot\Actions\ChatAction::restore',
        'permission_callback' => function ($request) {
            $auth = wpb_chatbot_app(\WPBulgaria\Chatbot\Auth\Factory\ChatsAuthFactory::class);
            $result = $auth->restore(get_current_user_id(), $request->get_param('id'), $request->get_param('chatbot_id'));
            if ($auth->hasError() && !$result) {
                return new \WP_Error("unauthorized", $auth->getError()->getMessage(), array("status" => 401));
            }
            return $result;
